...
if (!supabaseUrl || !supabaseAnonKey) {
  console.error('❌ Supabase env variables missing! Check NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in Vercel Environment Variables.')
}
...
export type Asset = {
  ...
  category_id: string | null; location_id: string | null
  status: 'Tersedia' | 'Digunakan' | 'Rusak'
  notes: string | null; created_at: string; updated_at: string
  category_name?: string; location_name?: string
}
export type AssetLog = {
  id: string; asset_id: string; asset_name: string; serial_number: string
...
export const deleteCategory = (id: string) => supabase.from('categories').delete().eq('id', id)
...
export const deleteLocation = (id: string) => supabase.from('locations').delete().eq('id', id)

export const getAssets = () =>
  supabase.from('assets').select(`*, category:categories(name), location:locations(name)`).order('created_at', { ascending: false })
export const createAsset = (data: {
  name: string; serial_number: string; category_id: string
  location_id: string; status: string; notes: string
...
export const deleteAsset = (id: string) => supabase.from('assets').delete().eq('id', id)

export const getAssetLogs = () =>
  supabase.from('asset_logs').select('*').order('changed_at', { ascending: false }).limit(100)
