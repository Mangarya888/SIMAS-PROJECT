        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#1e4080]/40">
              ...
              <th className="text-left px-5 py-3.5 text-xs font-mono text-slate-500 uppercase tracking-widest">Aksi</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Kategori' : 'Tambah Kategori'}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="Contoh: Router" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className={inputClass} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1.5 font-medium">Deskripsi</label>
            <textarea placeholder="Deskripsi singkat (opsional)" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={3} className={inputClass + ' resize-none'} />
          </div>
