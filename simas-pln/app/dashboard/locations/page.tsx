      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Lokasi' : 'Tambah Lokasi'}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="Contoh: Gudang A" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className={inputClass} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1.5 font-medium">Alamat</label>
            <textarea placeholder="Alamat lengkap (opsional)" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} rows={3} className={inputClass + ' resize-none'} />
          </div>
